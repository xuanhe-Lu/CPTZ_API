package com.ypm.bean;

import java.io.Serializable;
import java.math.BigDecimal;

public class ProdInfo implements Serializable {

	private static final long serialVersionUID = -5703135319907802630L;

	private long pid = 0;

	private long rid = 0;

	private int cid = 0, tid = 0;

	private String aa;

	private int ab, ac, ad;

	private String ae, af;

	private int ag, ah, ai, aj, ak, al;

	private BigDecimal am = BigDecimal.ZERO;

	private BigDecimal an = BigDecimal.ZERO;

	private int ao;

	private int ap, at, au;

	private String ba, bg, ca;

	private BigDecimal ma = BigDecimal.ZERO;

	private BigDecimal mb = BigDecimal.ZERO;

	private BigDecimal mc = BigDecimal.ZERO;

	private BigDecimal md = BigDecimal.ZERO;

	private int sa, sb;

	private String way;

	private int adj = 0;

	private BigDecimal adk = BigDecimal.ZERO;

	private BigDecimal ads = BigDecimal.ZERO;

	private long gmtA, gmtB, gmtC;

	private int state = 0;

	private long stime = 0;

	private long time = 0;

	public long getPid() {
		return pid;
	}

	public void setPid(long pid) {
		this.pid = pid;
	}

	public long getRid() {
		return rid;
	}

	public void setRid(long rid) {
		this.rid = rid;
	}

	public int getCid() {
		return cid;
	}

	public void setCid(int cid) {
		this.cid = cid;
	}

	public int getTid() {
		return tid;
	}

	public void setTid(int tid) {
		this.tid = tid;
	}

	public String getAa() {
		return aa;
	}

	public void setAa(String aa) {
		this.aa = aa;
	}

	public int getAb() {
		return ab;
	}

	public void setAb(int ab) {
		this.ab = ab;
	}

	public int getAc() {
		return ac;
	}

	public void setAc(int ac) {
		this.ac = ac;
	}

	public int getAd() {
		return ad;
	}

	public void setAd(int ad) {
		this.ad = ad;
	}

	public String getAe() {
		return ae;
	}

	public void setAe(String ae) {
		this.ae = ae;
	}

	public String getAf() {
		return af;
	}

	public void setAf(String af) {
		this.af = af;
	}

	public int getAg() {
		return ag;
	}

	public void setAg(int ag) {
		this.ag = ag;
	}

	public int getAh() {
		return ah;
	}

	public void setAh(int ah) {
		this.ah = ah;
	}

	public int getAi() {
		return ai;
	}

	public void setAi(int ai) {
		this.ai = ai;
	}

	public int getAj() {
		return aj;
	}

	public void setAj(int aj) {
		this.aj = aj;
	}

	public int getAk() {
		return ak;
	}

	public void setAk(int ak) {
		this.ak = ak;
	}

	public int getAl() {
		return al;
	}

	public void setAl(int al) {
		this.al = al;
	}

	public BigDecimal getAm() {
		return am;
	}

	public void setAm(BigDecimal am) {
		this.am = am;
	}

	public BigDecimal getAn() {
		return an;
	}

	public void setAn(BigDecimal an) {
		this.an = an;
	}

	public int getAo() {
		return ao;
	}

	public void setAo(int ao) {
		this.ao = ao;
	}

	public int getAp() {
		return ap;
	}

	public void setAp(int ap) {
		this.ap = ap;
	}

	public int getAt() {
		return at;
	}

	public void setAt(int at) {
		this.at = at;
	}

	public int getAu() {
		return au;
	}

	public void setAu(int au) {
		this.au = au;
	}

	public String getBa() {
		return ba;
	}

	public void setBa(String ba) {
		this.ba = ba;
	}

	public String getBg() {
		return bg;
	}

	public void setBg(String bg) {
		this.bg = bg;
	}

	public String getCa() {
		return ca;
	}

	public void setCa(String ca) {
		this.ca = ca;
	}

	public BigDecimal getMa() {
		return ma;
	}

	public void setMa(BigDecimal ma) {
		this.ma = ma;
	}

	public BigDecimal getMb() {
		return mb;
	}

	public void setMb(BigDecimal mb) {
		this.mb = mb;
	}

	public BigDecimal getMc() {
		return mc;
	}

	public void setMc(BigDecimal mc) {
		this.mc = mc;
	}

	public BigDecimal getMd() {
		return md;
	}

	public void setMd(BigDecimal md) {
		this.md = md;
	}

	public int getSa() {
		return sa;
	}

	public void setSa(int sa) {
		this.sa = sa;
	}

	public int getSb() {
		return sb;
	}

	public void setSb(int sb) {
		this.sb = sb;
	}

	public String getWay() {
		return way;
	}

	public void setWay(String way) {
		this.way = way;
	}

	public int getAdj() {
		return adj;
	}

	public void setAdj(int adj) {
		this.adj = adj;
	}

	public BigDecimal getAdk() {
		return adk;
	}

	public void setAdk(BigDecimal adk) {
		this.adk = adk;
	}

	public BigDecimal getAds() {
		return ads;
	}

	public void setAds(BigDecimal ads) {
		this.ads = ads;
	}

	public long getGmtA() {
		return gmtA;
	}

	public void setGmtA(long gmtA) {
		this.gmtA = gmtA;
	}

	public long getGmtB() {
		return gmtB;
	}

	public void setGmtB(long gmtB) {
		this.gmtB = gmtB;
	}

	public long getGmtC() {
		return gmtC;
	}

	public void setGmtC(long gmtC) {
		this.gmtC = gmtC;
	}

	public int getState() {
		return state;
	}

	public void setState(int state) {
		this.state = state;
	}

	public long getStime() {
		return stime;
	}

	public void setStime(long stime) {
		this.stime = stime;
	}

	public long getTime() {
		return time;
	}

	public void setTime(long time) {
		this.time = time;
	}
}
