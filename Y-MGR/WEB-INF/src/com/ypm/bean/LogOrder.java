...
public class LogOrder implements Serializable {
	...
	private static final long serialVersionUID = -3763403284366832504L;
	...
	/** 汇总收益信息 */
	public void execute(int ah, int day) {
		...
		if (tme.compareTo(BigDecimal.ZERO) >= 1) {
			...
			this.setYma(tma.add(tmg).add(tmc).subtract(tmd).add(tmf));
			if (tx >= 1) {
				this.setYmb(VeRule.income(tma, rate.add(tme), tx));
			}
		} else {
			this.setYma(tma.add(tmg).add(tmc).subtract(tmd));
			if (tx >= 1) {
